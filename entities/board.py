...
class Board:
    def __init__(self, screen_width: int, screen_height: int, rows: int, cols: int):
        ...
        if total_cells % 2 != 0:
            raise ValueError("Board must contain an even number of cells.")
        ...
        values = self.generate_card_values()
    ...
    def draw_debug(self, screen: pygame.Surface) -> None:
        """
        Draws the grid rectangles for debugging purposes.
        """
    ...
    def generate_card_values(self) -> list[int]:
        """
